  ...
  function MetricsProvider(screen) {
    ...
    var setupAggregation =
      function setupAggregation() {
        ...
        this.highestAggregationKey = this.aggregationLevels.slice(-1)[0];
        ...
        this._lastAggregation = this._startTime;
      };
    ...
    screen.on("zoomGraphs", function zoomGraphs(zoom) {
      // apply zoom delta while staying in boundaries
      this.zoomLevel =
        _.clamp(this.zoomLevel + zoom, -1, this.aggregationLevels.length - 1);
...
var aggregateMetrics =
  function aggregateMetrics(currentTime, metricData) {
    ...
    /**
     * Reclaim no longer necessary metadata storage space.  Once the highest
     * aggregation level configured has been computed, the metadata for all
     * those elements are no longer necessary and be reclaimed.
     *
     * @param {Object[]} rows
     * The array reference.
     *
     * @param {Number} startIndex
     * The start of the array where metadata can be discarded.
     *
     * @param {Number} endIndex
     * The end of the array where metadata can be discarded.
     *
     * @returns {void}
     */
    var reclaimMetadataStorage =
      function reclaimMetadataStorage(rows, startIndex, endIndex) {
        for (var rowIndex = startIndex; rowIndex <= endIndex; rowIndex++) {
          delete rows[rowIndex].__timeIndices;
        }
      };
    ...
    var processRow = function processRow(row, rowIndex, rows) {
      ...
      // get the time index of the aggregate (this is precomputed metadata)
      var thisTimeIndex = row.__timeIndices[aggregateKey];
      ...
      if (thisTimeIndex !== lastTimeIndex) {
        ...
        if (lastTimeIndex !== undefined) {
          ...
          addMissingTimeSlots.call(this, thisTimeIndex, lastTimeIndex);
          ...
          // we can reclaim some storage now since the time metadata is not needed anymore
          if (aggregateKey === this.highestAggregationKey) {
            reclaimMetadataStorage(
              rows,
              this._aggregation[aggregateKey].nextAggregateIndex,
              rowIndex - 1
            );
    ...
    for (aggregateKey in this._aggregation) {
      ...
      for (
        var rowIndex = this._aggregation[aggregateKey].lastIndex;
      ...
      // remember where we will begin again for this aggregate level
      this._aggregation[aggregateKey].lastIndex = this._metrics.length;
...
/**
 * Given a metric data point and a point in time, apply time metadata.
 *
 * @param {Object} metric
 ...
 *
 * @returns {Object}
 * The metric data point object is returned with time metadata applied.
 */
var applyMetricTimeMetadata =
  function applyMetricTimeMetadata(metric, currentTime) {
    var metadata = {
      __timeIndices: {}
    };

    // for all the aggregation levels, stamp each metric with its logical time indices
    for (var aggregateKey in this._aggregation) {
      // take the difference of now and start then divide by units of time and
      // drop any remainder; this is the logical time index for that unit of measure
      metadata.__timeIndices[aggregateKey] =
        Math.floor((currentTime - this._startTime) / +aggregateKey);
    }

    // now apply the metric data to the time metadata
    return Object.assign(metadata, metric);
  };
...
MetricsProvider.prototype._onMetrics =
  function _onMetrics(data) {
    ...
    // attach the current clock reading and metedata to the metrics
    this._metrics.push(applyMetricTimeMetadata.call(this, data, currentTime));
