 ...
 */
MetricsProvider.prototype.getTimeIndexLabel =
  function getTimeIndexLabel(timeIndex, aggregateTimeUnits) {
    ...
    return _.reduce(timeElements, function (prev, curr, index) {
      switch (curr.units) {
      case "s":
        return ":" + lpad(curr.value, DIGITS_PER_UNIT, "0");
      case "m":
      case "h":
        if (index < timeElements.length - 1) {
          return (curr.units === "m" ? ":" : " ") + lpad(curr.value, DIGITS_PER_UNIT, "0") + prev;
        } else {
...
MetricsProvider.prototype.getXAxis = function getXAxis(limit) {
  ...
  for (var timeIndex = limit - 1; timeIndex >= 0; timeIndex--) {
    xAxis.push(this.getTimeIndexLabel(timeIndex, +this.zoomLevelKey || TIME_SCALES[1].divisor));
  }
